...
```
vault research | research | find sources | find information about | research topic
```
...
`WebSearch`, `WebFetch` (or equivalent), `Read`, `Write`, `Bash`
...
| Distilled transcript extract(s) | `memory/raw/YYYY/MM/YYYY-MM-DD-<slug>.md` (retrieved transcripts only, via `distil-transcript`) |
...
- Retrieved transcripts are the exception: never retain an unprocessed transcript body in `memory/research/`. Route it through `distil-transcript` so the processed extract lands in `memory/raw/` instead.
